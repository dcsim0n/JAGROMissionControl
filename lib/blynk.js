...
exports.start = function (){
  ...
  b1.on('write', function(data){
   // When blynk button pressed, send relay change message 
    console.log("Sending realy command: 1, ", data);
  ...
  blynk.on('connect', ( ) => {
    ...
    mqtt.client.on('message', function(topic, message){
      const incommina = mqtt.client.creatJagroMessage(topic,message);
      if(message.ioType === 'relay'){
        if(message.ioPin === '1'){
          l1.write( parseInt( incomming.content ));
        }
        
        if(message.ioPin === '2'){
          l2.write( parseInt( incomming.content ));
        }
