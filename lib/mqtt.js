...
var client  = mqtt.connect(process.env.MQTT_ADDRESS, { username: process.env.MQTT_USER, password: process.env.MQTT_PASS, reconnectPeriod: 1000 });
...
class JagroMessage {
  constructor (topic, message){
    ...
    assert(topic.length >= 1, 'Topic string is empty!');
    this.domain = null;
    this.node = null;
    this.ioType = null;
    this.ioPin = null;
    ...
    assert(topicParts.length >= 2,'JagroMessage must have a domain and node, check your topic string!');
...
exports.createJagroMessage = function(topic,message) {
  return new JagroMessage(topic,message);
}
